class Solution {
    ...
    static int maxOfSubscribe, maxOfCost;
    ...
    public void findResult(int index, int len, int[] discounts,int[][] users, int[] emoticons) {
        if (index == len) {
            int subscribe = 0, cost = 0;
            for(int[] user: users){
        ...
        for(int i = 0; i < 4; i++) {
            ...
            findResult(index + 1, len, discounts, users, emoticons);
        }
    }
}